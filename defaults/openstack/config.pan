...
@use{
    type = string
    default = localhost
    note = Name of the engine node. This can be an opaque identifier. \
           It is not necessarily a hostname, FQDN, or IP address.
}
variable OPENSTACK_HEAT_HOST ?= 'localhost';
@use{
    type = string
    default = http
    note = This is the protocol used for communicating with OpenStack APIs, is set automatically based on the value of OPENSTACK_SSL
...
############################
# Magnum specific variable #
############################

@use{
    type = string
    default = localhost
    note = Name of this node. This can be an opaque identifier. It is not \
           necessarily a hostname, FQDN, or IP address. However, the node \
           name must be valid within an AMQP key, and if using ZeroMQ, a valid \
           hostname, FQDN, or IP address.
}
variable OPENSTACK_MAGNUM_HOST ?= 'localhost';
@use{
    type = hostname
    default = OPENSTACK_DB_HOST
    note = The host used for the Magnum database
}
variable OPENSTACK_MAGNUM_DB_HOST ?= OPENSTACK_DB_HOST;
@use{
    type = string
    default = magnum
    note = Username for Magnum to use to connect to it's database
}
variable OPENSTACK_MAGNUM_DB_USERNAME ?= 'magnum';
@use{
    type = string
    default = MAGNUM_DBPASS
    note = Password for Magnum to use to connect to it's database
}
variable OPENSTACK_MAGNUM_DB_PASSWORD ?= 'MAGNUM_DBPASS';
@use{
    type = string
    default = magnum
    note = The user to run Magnum under
}
variable OPENSTACK_MAGNUM_USERNAME ?= 'magnum';
@use{
    type = string
    default = MAGNUM_PASS
    note = The password to use for Magnum
}
variable OPENSTACK_MAGNUM_PASSWORD ?= 'MAGNUM_PASS';
@use{
    type = string
    default = magnum 
    note = The user to use as magnum domain admin
}
variable OPENSTACK_MAGNUM_DOMAIN_ADMIN_USERNAME ?= 'magnum_domain_admin';
@use{
    type = string
    default = MAGNUM_PASS
    note = The password for the magnum domain admin
}
variable OPENSTACK_MAGNUM_DOMAIN_ADMIN_PASSWORD ?= 'MAGNUM_DOMAIN_ADMIN_PASS';
@use{
    type = string
    default = ceph
    note =  The default docker volume_type to use for volumes used for docker \
            storage. To use the cinder volumes for docker storage, you need \
            to select a default value.
}
variable OPENSTACK_MAGNUM_DEFAULT_VOLUME_TYPE ?= 'ceph';
...
@use{
    ...
    note = Host name, for use in HATEOAS-style references Note: Typically \
           this would be the load balanced endpoint that clients would use \
           to communicate back with this service. If a deployment wants to \
           derive host from wsgi request instead then make this blank. \
           Blank is needed  to override default config value.
}
variable OPENSTACK_BARBICAN_HOST_HREF ?= 'http://localhost:9311';
@use{
    type = hostname
    default = OPENSTACK_DB_HOST
...
# IPHC #
########
variable TRANSPORT_URL ?= 'rabbit://' + OPENSTACK_RABBITMQ_USERNAME + ':' + OPENSTACK_RABBITMQ_PASSWORD + '@' + OPENSTACK_RABBITMQ_HOST + ':' + to_string(OPENSTACK_RABBITMQ_PORT);
variable OPENSTACK_KEYSTONE_MGMT_PROTOCOL ?= OPENSTACK_CONTROLLER_PROTOCOL;
variable KEYSTONE_CONTROLLER_AUTH ?= OPENSTACK_KEYSTONE_CONTROLLER_PROTOCOL + '://' + OPENSTACK_KEYSTONE_CONTROLLER_HOST + ':5000';
variable KEYSTONE_MGMT_AUTH ?= OPENSTACK_KEYSTONE_MGMT_PROTOCOL + '://' + OPENSTACK_KEYSTONE_MGMT_HOST + ':5000';
variable KEYSTONE_ADMIN_AUTH ?= OPENSTACK_KEYSTONE_MGMT_PROTOCOL + '://' + OPENSTACK_KEYSTONE_MGMT_HOST + ':35357';
variable KEYSTONE_PUBLIC_ENDPOINT ?= OPENSTACK_KEYSTONE_CONTROLLER_PROTOCOL + '://' + OPENSTACK_KEYSTONE_CONTROLLER_HOST + ':5000/v3';
variable KEYSTONE_MGMT_ENDPOINT ?= OPENSTACK_KEYSTONE_MGMT_PROTOCOL + '://' + OPENSTACK_KEYSTONE_MGMT_HOST + ':5000/v3';
variable OPENSTACK_DASHBOARD_PROTOCOL ?= 'https';
variable OPENSTACK_PROJECT ?= 'admin';
